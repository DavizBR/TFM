...
class ActionLogs(Action):
    def name(self) -> Text:
        return "action_logs"

    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        user_message = tracker.latest_message.get("text")
        bot_response = dispatcher.messages[0]["text"]

        db_path = "rasa.db"
        connection = sqlite3.connect(db_path)
        cursor = connection.cursor()
        cursor.execute("INSERT INTO conversation_history (user_message, bot_response) VALUES (?, ?)", (user_message, bot_response))
        connection.commit()
        connection.close()
